...
inline unsigned int BmpGetPixel12(const LCD_BMP12 *img, int screenX, int screenY, unsigned int fallback)
{
    if (!img || !img->data) return fallback;

    int bx = screenX - img->x0;
    int by = screenY - img->y0;

    if (bx < 0 || by < 0 || bx >= img->h || by >= img->w) return fallback;
    ...
    if ((idx & 1) == 0) 
    {
        return (unsigned int)(((img->data[off] << 4) | (img->data[off + 1] >> 4)) & 0x0FFF);
    } 
    else 
    {
        return (unsigned int)((((img->data[off + 1] & 0x0F) << 8) | img->data[off + 2]) & 0x0FFF);
    }
}
...
void LCDPutChar(char c, int x, int y, int size, int fColor, int bColor)
{
    ...
    // TRYB: przezroczystosc bez ruszania tła
    if (bColor == TRANSPARENT2)
    {
        for (i = nRows - 1; i >= 0; i--)
        {
            PixelRow = *pChar--;
            ...
            while (col < (int)nCols)
            {
                while (col < (int)nCols && (PixelRow & (0x80 >> col)) == 0) col++;
                int start = col;
                ...
                if (len > 0)
                {
                    ...
                    WriteSpiCommand(CASET);
                    WriteSpiData(y + start);
                    WriteSpiData(y + start + len - 1);
                    ...
                    int k;
                    for (k = 0; k < pairs; k++) {
                        // 2 piksele = 3 bajty (kolor ten sam)
                        WriteSpiData((fColor >> 4) & 0xFF);
                        WriteSpiData(((fColor & 0xF) << 4) | ((fColor >> 8) & 0xF));
                        WriteSpiData(fColor & 0xFF);
                    }
                    if (len & 1) {
                        // ostatni pojedynczy piksel = 2 bajty
                        WriteSpiData((fColor >> 4) & 0xFF);
                        WriteSpiData((fColor & 0xF) << 4); // 4 bity paddingu
                    }
    ...
    // TRYB: Tło stałe (konkretny kolor) lub pobieranie tła z grafiki.
    //W przypadku korzystania z pobierania tła z grafiki należy uzyć funkcji: LCDSetTransparentBackground()
        ...
        for (j = 0; j < (int)nCols; j += 2)
        {
            ...
            if ((PixelRow & Mask) == 0) {
                if (bColor == TRANSPARENT)
                    Word0 = BmpGetPixel12(&g_bg, sx0, sy0, 0x000);
                else
                    Word0 = (unsigned int)bColor;
            } else {
                Word0 = (unsigned int)fColor;
            }
            Mask >>= 1;
            ...
            if ((PixelRow & Mask) == 0) {
                if (bColor == TRANSPARENT)
                    Word1 = BmpGetPixel12(&g_bg, sx0, sy0 + 1, 0x000);
                else
                    Word1 = (unsigned int)bColor;
            } else {
                Word1 = (unsigned int)fColor;
            }
            Mask >>= 1;
...
//wersja zwracajaca offset w miejscu napisania tekstu
...
void LCDClearXY(int x, int y, int size, int w, int cColor)
{
    long i;
    int h;
    ...
    // Row address set (0x2B)  zakres inkluzywny
    WriteSpiCommand(PASET);
    WriteSpiData(x);
    WriteSpiData(x + h - 1);

    // Column address set (0x2A)  zakres inkluzywny
    WriteSpiCommand(CASET);
    WriteSpiData(y);
    WriteSpiData(y + w - 1);
    ...
    if (pixels & 1) {
        unsigned int Word0 = cColor;
        unsigned int Word1 = cColor;
        WriteSpiData((Word0 >> 4) & 0xFF);
        WriteSpiData(((Word0 & 0xF) << 4) | ((Word1 >> 8) & 0xF));
        WriteSpiData(Word1 & 0xFF);
    }
...
//uniwersalna funkcja ktora rysuje bmp o dowolnym rozmiarze (maks 132x132)
void LCDDrawBmp12(const unsigned char *bmpArr, int w, int h, int x0, int y0)
{
    long i;
    long pixels, bytes;

    if (!bmpArr || w <= 0 || h <= 0) return;
    ...
    bytes  = (3 * pixels + 1) / 2;  // ceil(12bpp)

    WriteSpiCommand(RAMWR);
    for (i = 0; i < bytes; i++) WriteSpiData(bmpArr[i]);
    WriteSpiCommand(NOP);
}
